    ...
    **kwargs
    ...
    if model_api == "statsmodels":
        for i, y in enumerate(y_variables):
            ...
            FitModel.append(
                model(formula=formulas[i], data=X_train.join(y_train[y])).fit()
            )
    ...
    model_dict = {"description": model_descr}
    ...
    if score:
        ...
        for i, var in enumerate(model_dict["y_variables"]):
            ...
            if score in train_opt:
                print(
                    "\t\tTraining\t{:.4f}".format(
                        model_dict["score"]["train"][i]
                    )
                )
            if score in test_opt:
                print(
                    "\t\tTest\t\t{:.4f}".format(model_dict["score"]["test"][i])
                )
            print()
        print("\n")
